...
export default function UserManagement() {
    ...
    const deleteUser = async(id)=> {
        try {
            console.log('hai')
            const { data } = await axios.post('http://localhost:4000/admin/delete-User/' + id, {}, { withCredentials: true })
            toast.error('user deleted', { position: "top-right" })
            if (data.status) {
                usermanagement()
            }
        } catch (error) {
                    ...
                    <TableBody>
                        {user.map((row, index) => (
                            ...
                            >
                                ...
                                <TableCell align="center"><Button onClick={()=>{deleteUser(row._id)}}>Delete</Button></TableCell>
